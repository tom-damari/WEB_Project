...
const ContactEmailInput = document.querySelector('#form-contactUs input[name="email"]');
const ContactPhoneInput = document.querySelector('#form-contactUs input[name="ILphone"]');
...
const onContactUsSubmit = (e) => {
    ...
    if (!validateFullName(ContactFullNameInput.value)) {
        errorMessage = 'נא להזין שם מלא תקין';
    ...
    if (errorMessage) { // if there is a wrong input then show error message
        ...
        // now after passing the contact validation check - submit the form
